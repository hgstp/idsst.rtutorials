...
Use `geom_line()` in the chunk below to recreate the graph above. The graph uses the `economics` dataset that comes with ggplot2 and maps the `date` and `unemploy` variables to the $x$ and $y$ axes.
...
```{r r2, echo = FALSE}
question("You've encountered whipsawing before in the tutorial on data basics. What does whipsawing indicate?",
...
You can now make all of the plots recommended in the tutorial on Exploratory Data Analysis. The following tutorials primer will teach you several strategies for dealing with overplotting, which often occurs when you have large data or low resolution data, and for customizing plots.
