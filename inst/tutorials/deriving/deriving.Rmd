...
In this case study, you will identify the most popular American names from 1880 to 2017. For this, we will use the entire `babynames` dataset, part of which we already worked with in the tutorial on `select()`, `filter()` and `arrange()`. While doing this, you will master three more dplyr functions:
...
The six dplyr functions help you work with these realities by isolating and revealing the information contained in your data. In fact, dplyr provides more than six functions for this grammar: dplyr comes with several functions that are variations on the themes of `select()`, `filter()`, `summarise()`, and `mutate()`. Each follows the same pipeable syntax that is used throughout dplyr. If you are interested, you can learn more about these peripheral functions in the [dplyr cheatsheet](https://raw.githubusercontent.com/rstudio/cheatsheets/main/data-transformation.pdf).
...
Use `number_ones` with `babynames` to recreate the plot below, which shows the popularity over time for every name in `number_ones` as the name for a boy.
