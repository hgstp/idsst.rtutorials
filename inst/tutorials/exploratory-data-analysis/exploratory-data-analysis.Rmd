...
EDA is an approach to analyzing data sets to summarize their main characteristics. When you do EDA, you:
...
EDA is an important part of any data analysis. You can use EDA to make discoveries about the world, or you can use EDA to ensure the quality of your data by asking questions about whether the data meets your standards or not.

### The EDA mindset

EDA is not a formal process with a strict set of rules. More than anything, EDA is a state of mind. During the initial phases of EDA, you should feel free to investigate every idea that occurs to you. Some of these ideas will pan out, and some will be dead ends. As your exploration progresses, you will gain insights that are worth writing down and passing on to others.
...
*   __Tabular data__ is a table of values, each associated with a variable and an observation. Tabular data is __tidy__ if each value is placed in its own cell, each variable in its own column, and each observation in its own row. 
    ...
    So far, all of the data that you've seen has been tidy. In real-life, most data isn't tidy, so we'll come back to these ideas again in the chapter on data wrangling and the following tutorials.
...
A variable is **categorical** if it can take only one of a finite set of values, which are categories, and it is not sensible to add, subtract, or take averages with those values. In R, categorical variables are usually saved as factors or character vectors. You can visualize the distribution of a categorical variable with a bar chart, like the one below. 
...
Don't worry if you cannot make or interpret a bar chart. In this tutorial, we'll survey several types of charts as we create a strategy for EDA. You'll learn how to build each type of chart in the following tutorials.

### Continuous variables

A variable is **continuous** (numeric) if it can take any of an infinite set of smooth, ordered values. Here, smooth means that if you order the values on a line, an infinite number of values would exist between any two points on the line. For example, an infinite number of values exists between 0 and 1, e.g. 0.9, 0.99, 0.999, and so on.

Continuous variables are a special case of the so-called **numerical** variables, which can take a wide range of numerical values, and it is sensible to add, subtract, or take averages with those values. A numerical variable that is not continuous is called discrete.
...
Many of the questions above will prompt you to explore a relationship *between* variables to see if one variable's values can explain another variable's values. We'll get to that shortly.

### Review 4 - Frequencies

The bar chart below visualizes the distribution of the `class` variable in the `mpg` data set, which comes in the ggplot2 package. The heights of the bars reveal how many cars in the dataset come from each class. 
...
To interpret the histogram, first look at the x-axis, which displays the lengths of eruptions recorded in the data. The range of the x-axis shows that the shortest eruptions lasted for about one minute and the longest for about five minutes. 

To see how many eruptions lasted for a specific length of time, find the length of time on the x-axis and then look at the height of the bar above the length of time. For example, according to the histogram, 30 eruptions lasted for about two minutes, but only three lasted for about three minutes (the height of the bar above two is 30, and the height of the bar above three is three).
...
The scatterplot below shows the relationship between the length of an eruption of Old Faithful and the wait time before the eruption (i.e., the time that passed between it and the previous eruption).
...
* Remember the definitions of _variables_, _values_, _observations_, _variation_, _covariation_, _categorical_, and _numerical (continuous/discrete)_. You'll see them again. Frequently.
...
The suggestions below can serve as a starting point for visualizing data. In the following tutorials, you will learn how to make each type of plot and how to use best practices and advanced skills when visualizing data.

```{r echo = FALSE, out.width = "100%"}
knitr::include_graphics("images/plots-table.png")
```
