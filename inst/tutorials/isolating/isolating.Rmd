...
babynames <- filter(babynames, year >= 1988)
```
...
The `babynames` dataset contains the number of people born in any year between 1880 and 2017 listed by name. We start by shortening the dataset to only include the last thirty years (i.e. all years between 1988 and 2017) to speed up the computation process.
