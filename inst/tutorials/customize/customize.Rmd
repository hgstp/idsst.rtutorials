...
The tutorial uses the ggplot2, dplyr, scales, and ggthemes packages.
...
To add a viridis palette, use `scale_color_viridis_c()` or `scale_fill_viridis_c()` for continuous and `scale_color_viridis_d()` or `scale_fill_viridis_d()` for discrete colors.
...
There exists the following color palettes: `"A"` (magma), `"B"` (inferno), `"C"` (plasma), `"D"` (viridis), `"E"` (cividis), `"F"` (rocket), `"G"` (mako), `"H"` (turbo).

To select a viridis color palette, set the `option` argument of `scale_color_viridis_c()` (or one of the others) to, e.g., `"C"` (plasma). 
...
```{r r13-solution}
p_cont + scale_color_viridis_c(option = "D")
```
